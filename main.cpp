...
int main() {
    // The project brief requires reading from an input file (e.g., "config.txt")
    // This example assumes the input format is one line per game: startNumber gridSize
     ifstream inputFile("reverse2048.txt");
    if (!inputFile.is_open()) {
        cerr << "Error: Unable to open config.txt for reading." <<  endl;
        return 1;
    }
    ...
    while (getline(inputFile, line)) {
        ...
        if (ss >> startNumber >> gridSize) {
            gameNumber++;
            cout << "Running Game " << gameNumber << " with start number " << startNumber
                     << " and grid size " << gridSize << "..." << endl;
